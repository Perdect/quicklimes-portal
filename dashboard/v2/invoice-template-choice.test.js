...
const TEMPLATES = ['classic', 'gst', 'modern', 'mono', 'compact'].map(id => ({ id }));
...
ok("nothing stored, firm declares nothing → classic",               resolve(null, '') === 'classic');
ok("LEGACY bare 'classic', firm declares gst → the firm's gst",     resolve('classic', 'gst') === 'gst');
ok("LEGACY bare 'classic', firm declares nothing → classic",        resolve('classic', '') === 'classic');
ok("LEGACY bare 'modern' was a real choice → modern, even for gst", resolve('modern', 'gst') === 'modern');
ok("EXPLICIT '!classic' beats the firm's gst (the owner chose it)", resolve('!classic', 'gst') === 'classic');
...
ok("an unknown explicit id with no firm format falls back to classic", resolve('!retired', '') === 'classic');
ok("an unknown legacy id falls back to the firm's format",          resolve('retired', 'gst') === 'gst');
ok("a firm declaring an unknown format still gets classic",         resolve(null, 'nope') === 'classic');
