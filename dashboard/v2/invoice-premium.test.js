...
ok('band geometry as measured on the reference: 82px tall on the 820 sheet, edge to edge, lockup 35px (≈24% of the width), 5.3% inset, title 17.5px', /\.band\{[^}]*padding:23px 5\.4% 23px 5\.3%;min-height:82px/.test(h) && /\.band \.lockup\{height:35px!important/.test(h) && /\.band \.ti \.w\{font-size:17\.5px/.test(h) && /@media print\{\.sheet\{max-width:none;padding:0 0 6mm;min-height:100vh/.test(h));
...
ok('totals block: value of goods with its arithmetic (tonnes to three decimals), IGST @ 5.00 % (HSN 25221000), total payable — and NO quantity row', /Value of goods \(\s*42 MT\s*× INR 5,500\.00\/Ton\)/.test(t) && /class="qtytot[^"]*">42 MT</.test(h) && /IGST @ 5\.00 % \(HSN 25221000\)/.test(t) && /INR 11,550\.00/.test(t) && /Total payable \(goods incl\. GST\)/.test(t) && /INR 2,42,550\.00/.test(t) && !/Total quantity/.test(t));
...
ok('supplier box lines: GSTIN · State, PAN · IEC, Mobile, E-mail', /PAN:<\/span><b>NLIPS9801K<\/b><\/span><span class="kv"><span>IEC:<\/span><b>NLIPS9801K<\/b>/.test(h) && /Mobile:<\/span><b>\+91 88750 20202, \+91 94607 67676<\/b>/.test(h));
