...
ok('the whole page is set at the reference\'s density: body 8.7px, box names 10.5px, table cells 8.7px, totals rows 5px pads, terms rows 6px pads', /body\{[^}]*font-size:8\.7px/.test(h) && /\.box \.nm\{font-size:10\.5px/.test(h) && /\.it td\{padding:7px 7px;[^}]*font-size:8\.7px/.test(h) && /\.tot td\{padding:5px 10px/.test(h) && /\.tc td\{padding:6px 0/.test(h));
...
ok('totals block: value of goods with its arithmetic (tonnes to three decimals), IGST @ 5.00 % (HSN 25221000), total payable — and NO quantity row', /Value of goods \(\s*42 MT\s*× INR 5,500\/Ton\)/.test(t) && /class="qtytot[^"]*">42 MT</.test(h) && /IGST @ 5\.00 % \(HSN 25221000\)/.test(t) && /INR 11,550\.00/.test(t) && /Total payable \(goods incl\. GST\)/.test(t) && /INR 2,42,550\.00/.test(t) && !/Total quantity/.test(t));
ok('table cells like the reference: centred headers, HSN 25221000 centred, quantity 42.000 bold centred, amount bold right', /th\{[^}]*text-align:center/.test(h) && /<td class="c">25221000<\/td>/.test(h) && /<td class="q">42\.000<\/td>/.test(h) && /<td class="r"><b>INR 2,31,000\.00<\/b>/.test(h));
...
ok('amount in words says what it includes', /Indian Rupees Two Lakh Forty Two Thousand Five Hundred Fifty Only \(inclusive of IGST\)/.test(t));
