...
ok('band geometry as measured on the reference (110 dpi → CSS px ×0.873): 79px tall on the 820 sheet, edge to edge, lockup 34px, 5.3% inset, title 16.5px', /\.band\{[^}]*padding:22px 5\.4% 22px 5\.3%;min-height:79px/.test(h) && /\.band \.lockup\{height:34px!important/.test(h) && /\.band \.ti \.w\{font-size:16\.5px/.test(h) && /@media print\{\.sheet\{max-width:none;padding:0 0 6mm;min-height:100vh/.test(h));
ok('the whole page is set at the reference\'s density: body 8.7px, box names 10.5px, table cells 8.7px, totals rows at a half-pixel pad and line-height 1 (owner, 18-09-2026: "reduce between spacing 70%"), terms rows 6px pads', /body\{[^}]*font-size:8\.7px/.test(h) && /\.box \.nm\{font-size:10\.5px/.test(h) && /\.it td\{padding:7px 7px;[^}]*font-size:8\.7px/.test(h) && /\.tot td\{padding:\.5px 10px;line-height:1;/.test(h) && /\.tc td\{padding:6px 0/.test(h));
...
ok('supplier address breaks like the reference: street + town, then State – PIN, India', /class="ad">Near Dharam Kanta Gotan Road, Borunda\nRajasthan – 342604, India</.test(h));
ok('a hand-broken address is kept as written; a state-less one gets just the PIN line', /class="ad">Plot 4\nMIDC Phase II\nPune</.test(R({ seller: Object.assign({}, seller, { address: 'Plot 4\nMIDC Phase II\nPune', state: '' }) })) && /class="ad">Plot 4, Pune\n411001, India</.test(R({ seller: Object.assign({}, seller, { address: 'Plot 4, Pune 411001', state: '', pin: '411001' }) })));
ok('no E-mail line in either box (the supplier e-mail is on the contact line, as in the reference); a buyer with an e-mail and a contact gets Attn. + E-mail; no Packing column when no line has packing', !/E-mail:/.test(h.slice(h.indexOf('From — Supplier'), h.indexOf('class="meta" style="margin-top:0"'))) && (function () { const x = R({ buyer: Object.assign({}, BASE.buyer, { email: 'buyer@phantom.in', contact: 'Mr. Aroof Ahmed' }) }); const b = x.slice(x.indexOf('To — Buyer')); return /Attn\.:<\/span><b>Mr\. Aroof Ahmed</.test(b) && /E-mail:<\/span><b>buyer@phantom\.in</.test(b); })() && !/Packing/.test(R({ packing: '' }).replace(/<[^>]+>/g, ' ')));
...
ok('the line: 42 · 5,500.00 · 2,31,000.00 — no INR prefix on money (owner, 18-09-2026)', /5,500\.00/.test(t) && /2,31,000\.00/.test(t) && !/INR /.test(h.slice(h.indexOf('<table class="it"'), h.indexOf('class="words"'))));
ok('totals block: value of goods with its arithmetic (tonnes to three decimals), IGST @ 5.00 % (HSN 25221000), total payable — and NO quantity row', /Value of goods \(\s*42 MT\s*× 5,500\/Ton\)/.test(t) && /class="qtytot[^"]*">42 MT</.test(h) && /IGST @ 5\.00 % \(HSN 25221000\)/.test(t) && /11,550\.00/.test(t) && /Total payable \(goods incl\. GST\)/.test(t) && /2,42,550\.00/.test(t) && !/Total quantity/.test(t));
ok('table cells like the reference: centred headers, HSN 25221000 centred, quantity 42.000 bold centred, amount bold right', /th\{[^}]*text-align:center/.test(h) && /<td class="c">25221000<\/td>/.test(h) && /<td class="q">42\.000<\/td>/.test(h) && /<td class="r"><b>2,31,000\.00<\/b>/.test(h));
ok('supplier box lines: GSTIN · State, PAN · IEC, Mobile — values in regular weight after grey labels', /PAN:<\/span><b>NLIPS9801K<\/b><\/span><span class="kv"><span>IEC:<\/span><b>NLIPS9801K<\/b>/.test(h) && /Mobile:<\/span><b>\+91 88750 20202, \+91 94607 67676<\/b>/.test(h));
ok('totals: the figure column fits its widest figure (width:1%, nowrap) so each label ends ~40px before its figure (20px was "too close") — owner, 18-09-2026', /\.tot td\.v\{[^}]*width:1%;white-space:nowrap;padding-left:30px\}/.test(h) && !/\.tot td\.l\{[^}]*width:\d+%/.test(h));
ok('amount in words says what it includes', /Rupees Two Lakh Forty Two Thousand Five Hundred Fifty Only \(inclusive of IGST\)/.test(t));
ok('a buyer contact prints as Attn.', /Attn\.:<\/span><b>Md\. Arman Khan Jony<\/b>/.test(R({ buyer: Object.assign({}, BASE.buyer, { contact: 'Md. Arman Khan Jony' }) })));
...
ok('amount in words as "Rupees … Only" — no "Indian" (owner, 18-09-2026)', /Amount in words:\s*Rupees Two Lakh Forty Two Thousand Five Hundred Fifty Only/.test(t) && !/Indian Rupees/.test(t));
ok('terms as a two-column list — a Label: text term prints its label as the key, an unlabelled one its number (1., 3.) — the jurisdiction term included', /class="k">1\.</.test(h) && /class="k">3\.</.test(h) && /class="k">Payment Terms</.test(R({ seller: Object.assign({}, seller, { terms: ['Payment Terms: 30 days net.'] }) })) && /Subject to 'RAJASTHAN' Jurisdiction only\./.test(t));
...
ok('the seal text is fitted to its arc (a long name is compressed, never clipped) and both arcs sit on the r=37 ring', (function () { const si = h.indexOf('<svg class="seal"'); const sv = h.slice(si, h.indexOf('</svg>', si)); const long = R({ seller: Object.assign({}, seller, { name: 'DESHWALI MINERALS AND CHEMICALS PRIVATE LIMITED' }) }); const li = long.indexOf('<svg class="seal"'); const lv = long.slice(li, long.indexOf('</svg>', li)); return /a37,37 0 0,1 74,0/.test(sv) && /a37,37 0 0,0 74,0/.test(sv) && /textLength="[\d.]+" lengthAdjust="spacingAndGlyphs"/.test(lv) && !/textLength/.test(sv.slice(0, sv.indexOf('sealBot', sv.indexOf('<text')))); })());
ok('on paper the sheet fills the page: signature block pushed to the bottom (print rule)', /@media print\{\.sheet\{[^}]*min-height:100vh[^}]*display:flex;flex-direction:column\}\.sheet \.in\{flex:1[^}]*\}\.sheet \.in \.close\{margin-top:auto/.test(h));
...
ok('footer: invoice no., date, "tax invoice under the CGST Rules, 2017" — no E&OE (owner, 18-09-2026)', /Invoice 2\/2026-27 · 18-09-2026 · This is a tax invoice under the CGST Rules, 2017\./.test(t) && !/Errors and omissions/.test(t));
...
ok('a Kg line priced per Ton: Qty (Kg), Rate / Ton, the conversion note, 40,545.00', /Qty \(Kg\)/.test(kg) && /Rate \/ Ton/.test(kg) && /7,650 Kg = 7\.65 Ton × ₹5,300\.00/.test(kg) && /40,545\.00/.test(kg) && !/4,05,45,000/.test(kg));
...
ok('multi-line: both items, the Kg line priced through units-core (40,545.00), the charge as its own row with HSN 9965', /Quick Lime Powder/.test(ml) && /Quick Lime Lumps/.test(ml) && /40,545\.00/.test(ml) && /Freight — Borunda to Haroa/.test(ml) && /9965/.test(ml) && /Charges[\s\S]*12,000\.00/.test(ml));
