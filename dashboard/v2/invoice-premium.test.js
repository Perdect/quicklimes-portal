...
ok('the band carries the FIRM\'S logo printed white, with the diamond facets drawn over it in navy (logoMark gem), and the two-line wordmark', /class="lg"[^>]*><img src="[^"]*deshwali-logo\.png"[^>]*filter:brightness\(0\) invert\(1\)/.test(h) && /viewBox="0 0 1000 655"[\s\S]*stroke="#0E2A47"/.test(h.slice(h.indexOf('class="lg"'), h.indexOf('class="n"'))) && /class="n">DESHWALI<br>MINERALS</.test(h));
ok('another firm\'s logo gets a clean white silhouette (no facet lines); a firm with no logo gets the vector gem', !/viewBox="0 0 1000 655"/.test(R({ seller: Object.assign({}, seller, { logoMark: '', logo: '/v2/gotan-logo.png' }) })) && /<svg class="gem"/.test(R({ seller: Object.assign({}, seller, { logo: '' }) })));
