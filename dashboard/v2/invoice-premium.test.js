...
ok('navy band with the firm name, the product line in gold, TAX INVOICE and the copy label', /class="band"/.test(h) && /DESHWALI MINERALS/.test(t) && /Manufacturer &amp; Exporter of Premium Quick Lime/.test(h) && /TAX INVOICE/.test(t) && /Original for Recipient/.test(t));
ok('gold rule under the band', /border-bottom:3px solid #C9A227/.test(h));
ok('the logo prints WHITE on the navy band (same colour as the letterhead) and the name is a two-line wordmark', /\.band \.co img\{filter:brightness\(0\) invert\(1\)/.test(h) && /class="n">DESHWALI<br>MINERALS</.test(h));
ok('contact line: website · e-mail · phones', /www\.deshwaliminerals\.com/.test(t) && /deshwaliminerals@gmail\.com/.test(t) && /8875020202, 9460767676/.test(t));
...
ok('the seal: firm name on the top arc, the stamp\'s own line on the bottom arc, stars and dash-dot marks — never an invented registration', (function () { const sv = h.slice(h.indexOf('<svg class="seal"'), h.indexOf('</svg>')); return /sealTop[\s\S]*DESHWALI MINERALS/.test(sv) && /sealBot[\s\S]*BORUNDA, GOTAN, RAJASTHAN/.test(sv) && (sv.match(/★/g) || []).length === 2 && /<line /.test(sv) && !/Regd\. No|Reg\. No|GSTIN/.test(sv); })());
