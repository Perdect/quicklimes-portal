...
ok('registry is gst, modern, business, detailed, industrial, premium, blueink — in that order', T.TEMPLATES.map(t => t.id).join(',') === 'gst,modern,business,detailed,industrial,premium,blueink');
ok('the rejected designs are gone', !T.TEMPLATES.some(t => /mono|compact|classic/.test(t.id)));
ok('modern, business, detailed, industrial and blueink are accentable (the colour picker applies)', T.TEMPLATES.filter(t => t.accentable).map(t => t.id).join(',') === 'modern,business,detailed,industrial,blueink');
