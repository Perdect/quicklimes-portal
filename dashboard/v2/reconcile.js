...
function openLink(tid) {
  ...
  b.classList.add('rc-side');   /* same reason as openSplit: keep the table visible */
...
function openSplit(tid) {
  ...
  const paint = () => {
    ...
    const over = rem() < -0.5;
    /* Two panes: LEFT is the decision so far (payment, running total, the bills
       already picked); RIGHT is everything still available to pick. Keeping
       them side by side means the total never scrolls away while you hunt
       through candidates — the one number you must watch stays pinned. */
    b.querySelector('.rc-modal-b').innerHTML = `
      <div class="rc-two">
        <div class="rc-two-l">
          ...
          <div class="rc-split-add-h">Matched against ${picks.length ? '(' + picks.length + ')' : ''}</div>
          <div class="rc-al-list">${rows || '<div class="rc-none" style="padding:10px 0">Pick bills on the right to split this payment across them.</div>'}</div>
        </div>
        <div class="rc-two-r">
  ...
  /* Dock right instead of centring: the statement row being matched stays
     visible on the left, which is the whole point of a match picker. */
  b.classList.add('rc-side');
